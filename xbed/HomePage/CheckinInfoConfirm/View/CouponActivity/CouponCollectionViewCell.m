...
- (void)setCouponModel:(CouponCardModel *)couponModel
{
    _couponModel = couponModel;
    
    _lbTitle.text = couponModel.title;
    
    
    _lbCondition.text = [NSString stringWithFormat:@"有效期：%ld.%02ld.%02ld－%ld.%02ld.%02ld", couponModel.beginTimestamp.year, couponModel.beginTimestamp.month, couponModel.beginTimestamp.day, couponModel.endTimestamp.year, couponModel.endTimestamp.month, couponModel.endTimestamp.day];
    
    
    _lbDesc.text = couponModel.desc;
    _lbDesc.lineSpace = 5;
...
- (void)setAllowsSelection:(BOOL)allowsSelection
{
    ...
    _btnRadio.hidden = !allowsSelection;
}
...
- (void)setEnable:(BOOL)enable
{
    ...
    if(enable)
    {
        ...
        _btnRadio.hidden = !_allowsSelection;
        _lbCondition.textColor = Main_Text_Color;
        _lbDesc.textColor = Secondary_Text_Color;
    }
    else
    {
        _vTitle.backgroundColor = Gray_Color;
        _btnRadio.hidden = YES;
        _lbCondition.textColor = Secondary_Text_Color2;
